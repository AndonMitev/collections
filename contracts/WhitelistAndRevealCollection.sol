...
contract RevealCollection is ERC1155, Ownable, PaymentSplitter, EIP712 {
    ...
    function mint(address _redeemer, NFTVoucher calldata _voucher) public payable {
        require(whitelisted[_redeemer], "Not whitelisted");
        ...
        _mint(signer, _voucher.tokenId, 1, "");

        // transfer the token to the redeemer
        _safeTransferFrom(signer, _redeemer, _voucher.tokenId, 1, "");

        emit NFTMinted(signer, _redeemer, _voucher.tokenId, uri(_voucher.tokenId));
    }
    ...
    function _hash(NFTVoucher calldata voucher) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("NFTVoucher(uint256 tokenId,uint256 maxSupply)"),
                        voucher.tokenId,
                        voucher.maxSupply
                    )
                )
